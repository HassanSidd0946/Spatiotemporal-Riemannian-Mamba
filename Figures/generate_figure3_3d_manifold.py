...
# # command to download : modal volume get eeg-data-vol figures/figure3_3d_riemannian_manifold.png .
# # generate_figure3_3d_manifold.py
# # Phase 3.2 — Figure 3: 3D Riemannian Tangent-Space Manifold Scatter
# # IEEE Transactions / Nature Neuroscience-style publication figure
# #
# # WHAT THIS FIGURE SHOWS (please read before using it in the manuscript):
# #   This script projects the EA-ALIGNED tangent-space feature matrix
# #   (`/data/tangent_features_ea.npz`) into a 3-component PCA subspace and
# #   scatters a class-balanced subsample, colored by cognitive state
# #   (True vs. False Memory).
# #
# #   Important scope note: this is a single GLOBAL PCA fit on the full,
# #   pooled 29-subject tangent-feature matrix — it is a descriptive view of
# #   the EA-aligned input geometry, not a rendering of the Condition 4
# #   per-fold, per-subject PCA-subspace + shrinkage-calibration pipeline
# #   (that pipeline fits a fresh 35-dim PCA subspace on 28 training subjects
# #   for each of the 29 LOSO folds, and blends a local/global classifier —
# #   there is no single global subspace it produces that could be scattered
# #   this way). If the manuscript intends to claim this figure depicts the
# #   effect of Subspace Shrinkage Calibration specifically (e.g. a
# #   before/after rescue comparison), that would need a different, per-fold
# #   figure design — happy to build that as a follow-up. As written, this
# #   figure is accurately captioned as showing topological separability of
# #   the EA-aligned tangent-space manifold itself.
# #
# #   Because this is an exploratory/descriptive visualization (not a
# #   predictive LOSO evaluation), the scaler and PCA are fit on the full
# #   dataset. This is standard practice for this kind of manifold plot and
# #   carries none of the leakage risk that would apply to an accuracy claim.
# #
# # Usage: modal run generate_figure3_3d_manifold.py::main
# #        modal run generate_figure3_3d_manifold.py::inspect   (sanity-check inputs)
# # =============================================================================
...
# OUTPUT_PNG    = f"{FIGURES_DIR}/figure3_3d_riemannian_manifold.png"
# OUTPUT_PDF    = f"{FIGURES_DIR}/figure3_3d_riemannian_manifold.pdf"

# RANDOM_SEED      = 42
# N_PER_CLASS      = 600     # -> 1200 total epochs, exact class balance
# PCA_N_COMPONENTS = 3

# CLASS0_COLOR = "#E11D48"   # False Memory — deep coral/crimson
# CLASS1_COLOR = "#059669"   # True Memory  — emerald/teal (signature intervention tone)
# CLASS0_LABEL = "False Memory (Class 0)"
# CLASS1_LABEL = "True Memory (Class 1)"

# # Candidate key names tried, in order, for each array in the .npz archive.
# X_KEYS   = ["X_ts", "X", "features", "tangent_features", "tangent_vectors", "data"]
# Y_KEYS   = ["y", "labels", "label", "Y", "targets"]
# SUB_KEYS = ["subjects", "subject", "subject_ids", "subject_id", "groups", "sub_ids"]
...
#     # =========================================================================
#     # SECTION 3: LOAD TANGENT-SPACE FEATURES (flexible key detection)
#     # =========================================================================
#     if not os.path.exists(DATA_PATH):
#         raise FileNotFoundError(f"Required feature archive not found: {DATA_PATH}")

#     raw = np.load(DATA_PATH, allow_pickle=True)
#     print(f"Archive keys found: {raw.files}")

#     def pick_key(candidates, purpose):
#         for c in candidates:
#             if c in raw.files:
#                 return c
#         raise KeyError(
...
#             f"Run `modal run generate_figure3_3d_manifold.py::inspect` to "
#             f"see exact keys/shapes and update the candidate list."
#         )

#     X_KEY = pick_key(X_KEYS, "feature matrix")
#     Y_KEY = pick_key(Y_KEYS, "labels")
#     SUB_KEY = next((c for c in SUB_KEYS if c in raw.files), None)

#     print(f"Using keys → features: '{X_KEY}', labels: '{Y_KEY}', "
#           f"subjects: '{SUB_KEY}' (informational only, not required for this figure)")

#     X = raw[X_KEY].astype(np.float64)
#     y = raw[Y_KEY].astype(np.int64)

#     if X.ndim != 2:
#         print(f"⚠️  Feature array has shape {X.shape} (ndim={X.ndim}); "
#               f"flattening trailing dims into a single feature axis.")
#         X = X.reshape(X.shape[0], -1)

#     n_total, raw_dim = X.shape
#     unique_classes = sorted(np.unique(y).tolist())
#     print(f"X: {X.shape} | y: {y.shape} | classes present: {unique_classes}")

#     if SUB_KEY is not None:
#         subjects = raw[SUB_KEY]
#         print(f"Subjects present: {len(np.unique(subjects))} unique (from '{SUB_KEY}')")

#     if set(unique_classes) - {0, 1}:
#         raise ValueError(
#             f"Expected strictly binary labels {{0, 1}} (0=False Memory, "
#             f"1=True Memory), but found classes {unique_classes}."
#         )

#     # =========================================================================
#     # SECTION 4: STANDARDIZE + GLOBAL 3D PCA PROJECTION
#     #   Descriptive/exploratory manifold plot — fit on the full pooled
#     #   29-subject tangent-feature matrix (see scope note in header).
#     # =========================================================================
...
#     print("\n" + "="*70)
#     print("  FIGURE 3 — PCA PROJECTION SUMMARY")
#     print("="*70)
#     print(f"  Input tangent-vector dimensionality : {raw_dim}")
#     print(f"  PCA components                       : {PCA_N_COMPONENTS}")
#     for i, v in enumerate(evr, start=1):
#         print(f"  PC{i} explained variance             : {v*100:.2f}%")
#     print(f"  Cumulative explained variance        : {evr.sum()*100:.2f}%")
#     print("="*70 + "\n")

#     # =========================================================================
#     # SECTION 5: STRATIFIED CLASS-BALANCED SUBSAMPLE (600 + 600 = 1,200)
#     # =========================================================================
...
#         n_take = min(len(idx_class0), len(idx_class1), N_PER_CLASS)
#         print(
#             f"  ⚠️  Requested {N_PER_CLASS}/class but only "
#             f"{len(idx_class0)} (class 0) / {len(idx_class1)} (class 1) "
#             f"epochs available — sub-sampling {n_take}/class instead."
#         )

#     sel_class0 = rng.choice(idx_class0, size=n_take, replace=False)
#     sel_class1 = rng.choice(idx_class1, size=n_take, replace=False)
#     sample_idx = np.concatenate([sel_class0, sel_class1])

#     X_sample = X_pca[sample_idx]
#     y_sample = y[sample_idx]

#     print(f"  Plotted sample: {len(sample_idx)} epochs "
#           f"({int((y_sample == 0).sum())} False Memory, "
#           f"{int((y_sample == 1).sum())} True Memory)")

#     # =========================================================================
#     # SECTION 6: 3D PUBLICATION-GRADE SCATTER
#     # =========================================================================
#     plt.rcParams.update({
...
#     ax.scatter(
#         X_sample[mask0, 0], X_sample[mask0, 1], X_sample[mask0, 2],
#         c=CLASS0_COLOR, s=38, alpha=0.75,
#         edgecolor="#7F1D1D", linewidth=0.3, depthshade=True,
#         label=CLASS0_LABEL, zorder=2,
#     )
#     ax.scatter(
#         X_sample[mask1, 0], X_sample[mask1, 1], X_sample[mask1, 2],
#         c=CLASS1_COLOR, s=38, alpha=0.75,
#         edgecolor="#064E3B", linewidth=0.3, depthshade=True,
#         label=CLASS1_LABEL, zorder=2,
#     )

#     ax.view_init(elev=28, azim=135)

#     ax.set_xlabel("Principal Manifold Axis 1", fontsize=11.5, fontweight="bold", labelpad=14)
#     ax.set_ylabel("Principal Manifold Axis 2", fontsize=11.5, fontweight="bold", labelpad=14)
#     ax.set_zlabel("Principal Manifold Axis 3", fontsize=11.5, fontweight="bold", labelpad=10)

#     # ---- Light panel backgrounds + subtle grey gridlines ----
#     ax.xaxis.set_pane_color((0.97, 0.97, 0.98, 1.0))
#     ax.yaxis.set_pane_color((0.96, 0.96, 0.97, 1.0))
#     ax.zaxis.set_pane_color((0.95, 0.95, 0.96, 1.0))
#     for axis in (ax.xaxis, ax.yaxis, ax.zaxis):
#         axis._axinfo["grid"]["color"] = (0.82, 0.82, 0.85, 0.5)
#         axis._axinfo["grid"]["linewidth"] = 0.6

#     ax.set_title(
#         "Riemannian Tangent Space Manifold: 3D Topological Separation of Cognitive States",
#         fontsize=14, fontweight="bold", pad=22,
#     )

#     ax.legend(
#         loc="upper right", fontsize=10.5, frameon=True,
#         facecolor="white", edgecolor="#D4D4D8", framealpha=0.92,
#     )

#     fig.tight_layout()

#     # =========================================================================
#     # SECTION 7: EXPORT — 600 DPI PNG + VECTOR PDF
#     # =========================================================================
...
#     print("="*70)
#     print("  FIGURE 3 EXPORT CONFIRMATION")
#     print("="*70)
...
#     print(f"  Epochs plotted : {len(sample_idx)}")
...
#         "pc3_explained_variance"        : float(evr[2]),
...
# def main():
#     print("\n" + "="*70)
#     print("  Phase 3.2 — Figure 3: 3D Riemannian Tangent-Space Manifold Scatter")
#     print("  Class-Balanced 1,200-Epoch Subsample, Nature/IEEE Style")
#     print("="*70 + "\n")
...
#     print("="*70)
#     print(
#         "\n  Figure 3 generated and committed to eeg-data-vol.\n"
#         "  Retrieve with: modal volume get eeg-data-vol figures/figure3_3d_riemannian_manifold.png .\n"
#         "             or: modal volume get eeg-data-vol figures/figure3_3d_riemannian_manifold.pdf .\n"
#     )
