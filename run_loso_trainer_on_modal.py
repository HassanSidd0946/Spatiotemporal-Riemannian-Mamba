...
# # Dataset : /data/processed_eeg_all_subjects.npz  (11610, 62, 251)
# # GPU     : Modal A100
# #
...
# # ---------------------------------------------------------------------------
# # Docker image — CUDA 12.1 devel base so nvcc is available for Mamba kernels
# # ---------------------------------------------------------------------------
# loso_image = (
#     modal.Image.from_registry(
#         "nvidia/cuda:12.1.1-devel-ubuntu22.04",
#         add_python="3.11",                       # Modal injects Python 3.11
#     )
#     # ── Step 1: Build toolchain prerequisites ──────────────────────────────
#     .pip_install(
#         "packaging",
#         "wheel",
#         "setuptools>=68",
#         "ninja",                                 # Parallel kernel compilation
#     )
#     # ── Step 2: PyTorch + CUDA 12.1 wheels ────────────────────────────────
#     .pip_install(
#         "torch==2.2.0",
#         "torchvision==0.17.0",
#         extra_index_url="https://download.pytorch.org/whl/cu121",
#     )
#     # ── Step 3: Scientific / ML stack ─────────────────────────────────────
#     .pip_install(
#         "transformers==4.39.0",
#         "numpy<2",           # mamba-ssm 2.x needs numpy<2 for C extensions
#         "einops",
#         "scikit-learn",
#         "mne",
#         "tqdm",
#     )
#     # ── Step 4: Custom CUDA kernels (must compile against installed CUDA) ──
#     # CC/CXX force GCC so nvcc picks the right host compiler.
#     # --no-build-isolation prevents pip from spawning a clean build env that
#     # would lose the already-installed torch headers Mamba needs.
#     .run_commands(
...
#     # ── Deferred imports (execute inside the Modal container) ────────────────
#     import numpy  as np
#     import torch
...
#     X_np         = raw["X"].astype(np.float32)   # (11610, 62, 251)
#     y_np         = raw["y"].astype(np.int64)      # (11610,)
#     subjects_np  = raw["subjects"]                # (11610,)  strings like "01"
...
#     # ─────────────────────────────────────────────────────────────────────────
#     # 4-A  CovarianceLayer
#     # Computes a batch of regularized SPD covariance matrices.
#     # Input  : (B, C, T)  float32
#     # Output : (B, C, C)  SPD float32
#     # ─────────────────────────────────────────────────────────────────────────
...
#             # Remove temporal mean (band-centre each trial)
#             x = x - x.mean(dim=-1, keepdim=True)           # (B, C, T)
#             n = x.shape[-1]
#             cov = torch.bmm(x, x.mT) / (n - 1)             # (B, C, C)
#             # Tikhonov regularisation: keeps matrix strictly positive-definite
#             cov = cov + self.reg * self.eye.unsqueeze(0)    # (B, C, C)  SPD ✓
#             return cov

#     # ─────────────────────────────────────────────────────────────────────────
#     # 4-B  TangentSpaceLayer
#     # Maps each SPD matrix to the tangent space at the identity via matrix log,
#     # then vectorises the symmetric result (upper triangle).
#     # Input  : (B, C, C)  SPD
#     # Output : (B, C*(C+1)//2)  — 1953-dim for C=62
#     # ─────────────────────────────────────────────────────────────────────────
#     class TangentSpaceLayer(nn.Module):
#         def __init__(self, n_channels: int):
#             super().__init__()
#             idx = torch.triu_indices(n_channels, n_channels)   # Long tensor
#             self.register_buffer("triu_r", idx[0])
#             self.register_buffer("triu_c", idx[1])

#         def forward(self, S: torch.Tensor) -> torch.Tensor:
#             # Eigendecomposition (eigh assumes symmetric — correct for SPD)
#             L, V = torch.linalg.eigh(S)                        # (B,C), (B,C,C)
#             L    = torch.clamp(L, min=1e-7)                    # numerical floor
#             # logm(S) = V · diag(log λ) · Vᵀ
#             log_S   = V @ torch.diag_embed(torch.log(L)) @ V.mT  # (B, C, C)
#             # Vectorise upper triangle (symmetric → no info in lower half)
#             tangent = log_S[:, self.triu_r, self.triu_c]         # (B, D_tang)
#             return tangent

#     # ─────────────────────────────────────────────────────────────────────────
#     # 4-C  SpatiotemporalRiemannianMamba
#     #
#     # Dual-branch:
#     #   ① Spatial  — Riemannian covariance → tangent space → 2-layer MLP
#     #   ② Temporal — Linear embed → Mamba SSM stack (pre-norm residual)
#     #
#     # Branches fused by concatenation before the classifier head.
#     # ─────────────────────────────────────────────────────────────────────────
...
#             # Project from channel-dim to d_model at each timestep
#             self.temporal_in   = nn.Linear(n_channels, d_model)
#             # Stack of Mamba blocks with pre-norm residual connections
...
#             # — Spatial branch —
#             cov     = self.cov_layer(x)            # (B, C, C)
#             tang    = self.tangent_layer(cov)       # (B, D_tang)
#             s_feat  = self.spatial_proj(tang)       # (B, d_model)

#             # — Temporal branch —
#             h = x.permute(0, 2, 1)                 # (B, T, C)
#             h = self.temporal_in(h)                 # (B, T, d_model)
#             for mamba, norm in zip(self.mamba_blocks, self.mamba_norms):
#                 # Pre-norm residual: h ← h + Mamba(LayerNorm(h))
#                 h = h + mamba(norm(h))
#             t_feat = self.temp_drop(h.mean(dim=1))  # (B, d_model)

#             # — Fusion —
#             out = self.head(torch.cat([s_feat, t_feat], dim=-1))  # (B, n_classes)
#             return out

#     # ─────────────────────────────────────────────────────────────────────────
#     # 4-D  BaselineCNN  (EEGNet-inspired)
#     #
#     # Three blocks:
#     #   ① Temporal conv  — learns spectral filters (band-pass-like)
#     #   ② Depthwise conv — learns spatial filters per spectral filter
#     #   ③ Separable conv — pointwise feature mixing
#     # Global adaptive pool keeps the classifier dimension fixed regardless
#     # of input length, so no hard-coded n_times arithmetic needed.
#     # ─────────────────────────────────────────────────────────────────────────
#     class BaselineCNN(nn.Module):
#         def __init__(
#             self,
#             n_channels : int   = 62,
#             n_times    : int   = 251,   # unused after AdaptivePool but kept for docs
#             n_classes  : int   = 2,
...
#         ):
#             super().__init__()
#             F2 = F1 * D                 # = 16  total spatial filters
...
#                 nn.BatchNorm2d(F1),
#             )
#             # Block 2 — Depthwise spatial (collapses channel dimension)
...
#                 nn.Dropout(dropout),
#             )
#             # Collapse any remaining temporal width → fixed F2-dim vector
...
#             x = self.block2(x)          # (B, F2, 1, T//4)
#             x = self.block3(x)          # (B, F2, 1, ~T//32)
#             x = self.gap(x).flatten(1)  # (B, F2)
...
#         """Single training epoch with mixed-precision (bfloat16 on A100)."""
#         model.train()
...
#             # Gradient clipping prevents explosion in SSM state updates
...
#     # ── Hyper-parameters ──────────────────────────────────────────────────────
#     # TEST_FOLDS  = 3      # ← flip to len(unique_subjects) for a full 30-fold run
#     # N_EPOCHS    = 15
#     # BATCH_SIZE  = 64
#     # LR          = 1e-4
#     # WEIGHT_DECAY= 1e-4
...
#     TEST_FOLDS  = len(unique_subjects)    # <--- Change this to run ALL 29 subjects
#     N_EPOCHS    = 50
#     BATCH_SIZE  = 64
#     LR          = 1e-4
#     WEIGHT_DECAY= 1e-4
...
#     log.info(f"Epochs/fold   : {N_EPOCHS} | Batch: {BATCH_SIZE} | LR: {LR}")
...
#         # ── Normalise  (z-score per channel, fit on train only) ───────────────
#         # Reshape to (samples × time, channels) so StandardScaler treats
#         # each electrode as an independent feature with its own μ / σ.
#         n_tr, n_te = X_tr_raw.shape[0], X_te_raw.shape[0]

#         X_tr_2d = X_tr_raw.transpose(0, 2, 1).reshape(-1, N_CH)  # (n_tr*T, C)
...
#                     d_model=128, n_layers=3, n_classes=N_CLASSES,
#                 ),
#             ),
#             (
#                 "BaselineCNN",
...
#             )
#             # Cosine schedule: smoothly anneals LR from LR → 0 over all epochs
#             scheduler  = torch.optim.lr_scheduler.CosineAnnealingLR(
#                 optimizer, T_max=N_EPOCHS, eta_min=LR * 0.01
#             )
#             # GradScaler for mixed-precision (bfloat16 is lossless on A100)
...
#                 # Log at epoch 1, 5, 10, 15 to avoid wall-of-text
#                 if epoch in {1, 5, 10, N_EPOCHS}:
...
#                 "acc" : round(final_acc,  6),
#             }
#             log.info(f"  └── {model_name} FINAL  →  loss={final_loss:.4f}  acc={final_acc:.4f}")
...
#     log.info(f"\n{'═'*70}")
#     log.info("  LOSO BENCHMARK  —  3-FOLD SANITY RESULTS")
#     log.info(f"{'═'*70}")
...
#     log.info(f"{'═'*70}")
#     log.info("  When ready for the full 30-fold run:")
#     log.info("  → Set  TEST_FOLDS = len(unique_subjects)  and re-deploy.")
#     log.info(f"{'═'*70}\n")
...
# def main():
#     print("\n" + "="*70)
#     print("  BCI LOSO Trainer — Launching on Modal A100")
#     print("  SpatiotemporalRiemannianMamba  vs.  BaselineCNN")
#     print("="*70 + "\n")
...
#     print("="*70)
#     print(
#         "\n  To scale to all 30 folds, set  TEST_FOLDS = 30  in the script"
#         "\n  and re-run.  Estimated runtime on A100: ~8–12 hours.\n"
#     )
