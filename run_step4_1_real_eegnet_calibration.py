...
# #
# # ⚠️  MUST VERIFY BEFORE RUNNING — see CONFIG block below:
# #   - SFREQ (sampling rate of ds005189 epochs). KERN_LENGTH is derived from
# #     it (Lawhern et al. use kernLength ≈ sfreq // 2). If SFREQ is wrong,
# #     EEGNet's temporal receptive field is wrong and results are invalid.
# #   - RAW_DATA_PATH and the key names picked up by `_pick_key` — this
# #     script expects raw (epochs, channels, time) arrays, NOT the
# #     precomputed tangent_features_ea.npz used by Conditions 2-4.
# #
# # LEAKAGE CONTROLS:
...
# # Expected contents: an array of shape (n_epochs, n_channels, n_times),
# # integer labels, and subject IDs per epoch (mirrors the tangent_features
# # file's structure, just pre-tangent-mapping).
# RAW_DATA_PATH   = "/data/raw_epochs_ds005189.npz"
...
# SFREQ            = 250     # <-- SET THIS to ds005189's true sampling rate (Hz)
# N_CHANNELS       = 62      # matches prior scripts' 62-channel montage
# KERN_LENGTH      = SFREQ // 2   # Lawhern et al.: ~half the sampling rate
# # ────────────────────────────────────────────────────────────────────────────
...
#     if SFREQ not in (128, 160, 200, 250, 256, 500, 512, 1000):
#         log.warning(
#             f"⚠️  SFREQ={SFREQ}Hz is unusual for scalp EEG. Double-check this "
#             f"against ds005189's actual acquisition rate before trusting "
#             f"KERN_LENGTH={KERN_LENGTH} (and therefore all downstream results)."
#         )
